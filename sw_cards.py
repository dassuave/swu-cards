"""Core class for returning a card list for Star Wars Unlimited"""
...
def get_card_data(card_aspects):
    """Returns a Card List for Star Wars Unlimited"""
    ...
    try:
        ...
        # total_cards_data = data['total_cards']
        ...
        for card in cards_data:
            ...
            if (card_type != 'Leader' and card_type != 'Base' and variants == "Normal" and aspects):
                if (all(aspect in aspects for aspect in card_aspects.get("currentAspects")) and
                    all(aspect not in aspects for aspect in card_aspects.get("antiAspects"))):
                    display_cards_data.append(card)
        return display_cards_data, cards_data
    except requests.exceptions.RequestException as e:
        print('Error connecting to API:', e)
        return None, None

def custom_sort_key(card):
    """Sets the custom key for sorting cards"""
    aspects = card.get('Aspects', [])
    style_aspect_count = aspects.count(CURRENT_ASPECTS[0])
    return (style_aspect_count, int(card.get('Cost', 0)))

def show_card_data(shown_cards):
    """Formats card data for display"""
    sorted_cards = sorted(shown_cards, key=custom_sort_key)
    page_length = 3
    page_count = 1
    ...
    for index, card in enumerate(sorted_cards):
        ...
        cost = card.get('Cost')
        card_type = card.get('Type')
        print(f'{card_type} | {name} | Cost: {cost}{displayed_count}')
...
if __name__ == "__main__":
    display_cards, cards = get_card_data(
        {"currentAspects": CURRENT_ASPECTS, "antiAspects": ANTI_ASPECTS})
